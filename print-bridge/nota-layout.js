/**
 * Layout struk thermal 80mm — selaras dengan f_jualcetaknota.php (lebar 47 karakter).
 */
const WIDTH = 47;
const DEF = 1;
...
function buildNotaText(data) {
  ...
  const tglJual = data.tgl_jual || (data.tgltime ? String(data.tgltime).split(' ')[0] : '');

  lines.push(spasiStr('', DEF) + spasiCenter(nmToko, WIDTH));
  if (alToko) {
    lines.push(spasiCenter(alToko, WIDTH + DEF));
  }
  lines.push('');
  lines.push(spasiStr('', DEF) + 'No.Struk ' + spasiStr('', 5) + ':' + spasiStr(data.no_fakjual || '', 20));
  lines.push(spasiStr('', DEF) + 'Tanggal  ' + spasiStr('', 5) + ':' + spasiStr(gantitgl(tglJual), 10));

  if (data.nm_member) {
    lines.push(spasiStr('', DEF) + 'Member   ' + spasiStr('', 5) + ':' + spasiStr(data.nm_member, 30));
    const poinEarned = parseAmount(data.poin_earned);
    if (poinEarned > 0) {
      lines.push(
        spasiStr('', DEF) + 'Poin Dapat' + spasiStr('', 3) + ':' +
        spasiStr(formatNumber(poinEarned) + ' Poin', 30)
      );
    }
    const poinSaldo = parseAmount(data.poin_saldo);
    lines.push(
      spasiStr('', DEF) + 'Poin Saldo' + spasiStr('', 3) + ':' +
      spasiStr(formatNumber(poinSaldo) + ' Poin', 30)
    );
  }

  lines.push(spasiStr('', DEF) + '-----------------------------------------------');
  lines.push(spasiStr('', DEF) + 'No.' + spasiStr('', 5) + 'Nama Barang');
  lines.push(
    spasiStr('', DEF) + spasiStr('', 5) +
  ...
  (data.items || []).forEach((item, idx) => {
    ...
    lines.push(
      spasiStr('', DEF) + spasiNum(idx + 1, 3) + '.' + spasiStr('', 1) + (item.nmbrg || '')
    );
    lines.push(
      spasiStr('', 4 + DEF) +
      spasiNum(qty, 3) +
